...
# from nas.search_config import config
from nas.eval import cal_metrics
...
from yolo5_utils.pytorch_visual import Pytorch_Visual
...
class ENAS:
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.constraint_type = hyp.get('constraint_type') 
        if not self.constraint_type in self.valid_constraint_range.keys():
            self.invite_reset_constraint_type()
        self.efficiency_constraint = hyp.get('efficiency_constraint') # latency
        if not (
        ...
        self.efficiency_predictor = LatencyTable(device=self.constraint_type)
        self.accuracy_predictor = hyp.get('accuracy_predictor')
        self.arch_manager = ArchManager()
        self.num_blocks = self.arch_manager.num_blocks
        self.num_stages = self.arch_manager.num_stages

        self.population_size = hyp.get('pop_size')
        self.max_time_budget = hyp.get('niter')
        self.mutate_prob = hyp.get("mutate_prob", 0.1)
        self.parent_ratio = hyp.get("parent_ratio", 0.25)
        self.mutation_ratio = hyp.get("mutation_ratio", 0.5)
